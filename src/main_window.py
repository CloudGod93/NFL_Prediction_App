...
from src.data_handler import get_schedule, get_current_nfl_week, get_training_data
from src.model import NFLGamePredictor
from src.team_data import TEAM_NAMES
...
class GameListItem(QWidget):
    def __init__(self, away_team_abbr, home_team_abbr, asset_path='assets'):
        ...
        layout.addWidget(away_logo); layout.addWidget(away_name)
        layout.addWidget(at_label); layout.addWidget(home_logo)
...
class PredictionResultWidget(QWidget):
    def __init__(self, away_abbr, home_abbr, winner_abbr, confidence, asset_path='assets'):
        super().__init__()
        self.setStyleSheet("""
        ...
        away_name = QLabel(TEAM_NAMES.get(away_abbr, away_abbr))
        if away_abbr == winner_abbr: away_name.setObjectName("winner_name")
        at_label = QLabel("@")
        home_logo = self.create_logo_label(home_abbr, winner_abbr, asset_path)
        home_name = QLabel(TEAM_NAMES.get(home_abbr, home_abbr))
        if home_abbr == winner_abbr: home_name.setObjectName("winner_name")
        
        matchup_layout.addWidget(away_logo); matchup_layout.addWidget(away_name, 1)
        matchup_layout.addSpacerItem(QSpacerItem(10, 20, QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum))
        matchup_layout.addWidget(at_label)
        matchup_layout.addSpacerItem(QSpacerItem(10, 20, QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Minimum))
        matchup_layout.addWidget(home_logo); matchup_layout.addWidget(home_name, 1)
        ...
        confidence_bar = QWidget()
        bar_color = "#4CAF50" if confidence > 0.7 else "#FFC107" if confidence > 0.6 else "#013369"
        confidence_bar.setStyleSheet(f"background-color: {bar_color}; border-radius: 4px;")
        
        bar_layout.addWidget(confidence_bar, int(confidence * 100))
        bar_layout.addStretch(100 - int(confidence * 100))
        
        confidence_layout.addWidget(confidence_label); confidence_layout.addWidget(bar_container, 1)
        main_layout.addLayout(matchup_layout); main_layout.addLayout(confidence_layout)

    def create_logo_label(self, team_abbr, winner_abbr, asset_path):
        logo_label = QLabel(); logo_label.setFixedSize(50, 50)
        opacity = "1.0" if team_abbr == winner_abbr else "0.3"
...
class MainWindow(QMainWindow):
    ...
    def setup_predict_ui(self):
        ...
        current_year, current_week = get_current_nfl_week() # Use new function here
        ...
        self.week_combo.setCurrentIndex(current_week - 1) # Set to current week
        
        self.load_schedule_button = QPushButton("Load Schedule")
        top_layout.addWidget(QLabel("Year:")); top_layout.addWidget(self.year_combo)
        top_layout.addWidget(QLabel("Week:")); top_layout.addWidget(self.week_combo)
        top_layout.addWidget(self.load_schedule_button)
    ...
    def update_schedule_display(self):
        self.main_stack.setCurrentWidget(self.game_list_widget)
        self.statusBar().showMessage("Loading schedule...")
        self.game_list_widget.clear()
        year, week = int(self.year_combo.currentText()), self.week_combo.currentIndex() + 1
        
        self.schedule = get_schedule(year, week)
        if self.schedule.empty:
            self.statusBar().showMessage(f"No schedule found for {year} Week {week}.", 5000); return
    ...
    def update_training_progress(self, value, text): self.progress_bar.setValue(value); self.progress_label.setText(text)
    def training_finished(self): self.predictor.load_model(); self.update_training_ui_state(); self.update_prediction_button_state()
    ...
    def update_prediction_button_state(self):
    ...
    def display_results(self, results_df):
        ...
        if results_df.empty:
            ...
            for _, row in results_df.iterrows():
                result_card = PredictionResultWidget(row['away_team'], row['home_team'], row['winner'], row['confidence'])
    ...
    def set_prediction_buttons_enabled(self, enabled):
